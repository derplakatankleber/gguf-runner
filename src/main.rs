...
use crate::engine::io::{
    bf16_to_fp32, fp16_to_fp32, parse_gguf_file, read_f32_le, read_u16_le, read_u32_le,
};
...
pub(crate) use crate::engine::types::*;
pub(crate) use crate::engine::profiling::*;
pub(crate) use crate::engine::switches::*;
...
fn run() -> Result<(), String> {
    ...
    init_runtime_config_from_cli(&cli);
    ...
    let rayon_threads: Option<usize> = cli.threads;
    ...
    let mut config = vendors::build_config_from_gguf(&gguf, debug_mode)?;

    let mut tokenizer = engine::tokenizer::init_tokenizer_from_gguf(&gguf, &mut config, debug_mode)?;
    tokenizer.use_sentencepiece = config.is_gemma3;

    engine::runtime::apply_context_size_overrides(&mut config, context_size, debug_mode);
    if max_tokens == 0 || max_tokens > config.seq_len {
        max_tokens = config.seq_len;
    }

    if let Some(n_threads) = rayon_threads {
        engine::runtime::configure_rayon_threads(n_threads, debug_mode);
    }
    ...
    let weights = engine::weights::init_weights_from_gguf(&gguf, &config, debug_mode)?;

    let mut state = engine::runtime::malloc_run_state(&config);

    let use_chat_template = true;

    let mut prompt_tokens: Vec<i32> = if use_chat_template {
        vendors::encode_chat_prompt(&mut tokenizer, &config, &prompt, &system_prompt)
    } else {
        let mut t = Vec::new();
